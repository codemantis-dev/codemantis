...
<!-- Date: June 2026 | App Version: 1.6.0 -->

# CodeMantis -- the native macOS app Claude Code and Codex deserve

> **Complete User Guide -- v1.6.0.** CodeMantis is a native macOS desktop application that wraps the **Claude Code** CLI *and* the **OpenAI Codex** CLI in a modern, project-oriented UI. You pick which coding agent each session uses; the rest of the app -- sessions, activity feed, file viewer, terminals, SpecWriter, Implementation Guide, Self-Drive, Preflight, multi-AI assistant -- works the same way regardless of agent.
>
> This guide covers every feature, every button, every setting, and every keyboard shortcut. It was generated by reading the application's source code -- every description matches the actual implementation.
>
...
> - **SpecWriter capability handshake + Coverage panel + AUDIT-PATCH** (v1.1.11): SpecWriter probes your project for real capabilities (Supabase keys, BrowserMCP, LLM creds...) and live-fires them before writing; an automatic UI-completeness audit shows up in the Coverage tab; the "Patch spec & re-audit" button asks the model for a splice patch instead of regenerating from scratch.
> - **Self-Drive evidence vocabulary + loop guard + capability gating** (v1.1.11): the orchestrator parses the agent's verify replies against a typed evidence vocab, bounded retry on repeat-pattern failures, and verify items tagged `capability=<id>` auto-resolve as N/A when the capability is absent.
> - **AES-GCM encryption of AI provider API keys at rest** (v1.1.10).
> - **Persistent CLI session id, crash recovery + graduated wake recovery** (v1.1.8 / v1.1.10 / v1.5.x): force-quit sessions are resumable, post-suspend wake re-attaches live sessions and reloads if necessary, and the recovery UI now restores agent identity correctly.
> - **AgentAdapter abstraction internal refactor** (v1.2.0): the entire Claude path moved through a pluggable `AgentAdapter` trait so adding more agents in future stays a non-event.
...
- [Chapter 19B: Preflight & Mission Control](#chapter-19b-preflight--mission-control)
...
- [Chapter 27A: Settings -- Agents (per-task routing)](#chapter-27a-settings----agents-per-task-routing)
...
- **Your Ideas to AI-powered Specifications to Claude Code or Codex** -- Integrated to work.
...
| **Resume Session** | History | Reopen a recent session across all projects -- jumps straight back into the conversation. Restores either Claude Code (`--resume`) or Codex (`thread/resume`) sessions; an **agent badge** on each row tells you which CLI the session belongs to. |

### Agent Picker

On the Templates, Open Folder, and Clone tabs an **Agent Picker** sits between the form and the "Open Project" / "Clone & Open" button -- this is where you choose which coding-agent CLI the project's first session will use:

- **Claude Code** (Anthropic's CLI -- uses your Claude Pro/Max subscription).
- **OpenAI Codex** (ChatGPT-bundled coding agent -- uses your ChatGPT subscription).

Each option is a radio row with a tagline. The picker auto-collapses to a static label (e.g. *"Agent: Claude Code · Add OpenAI Codex: npm install -g @openai/codex"*) when only one CLI is installed on disk, since a one-option radio is pointless. If neither is installed, the picker is hidden entirely -- the Welcome Screen handles install guidance instead.

Selection is persistent for the lifetime of the UI store (`uiStore.selectedAgentId`) and applies to every new session you create from that point on. You can change it at any time in **Settings → Agents** (Chapter 28A) or by opening the picker again before creating a session.

Codex's *auth* state is **not** checked here; the spawn path runs `codex login status` itself and surfaces an actionable error with a docs link if you are signed out. In-app `codex login` is deferred -- run it once in a terminal.
...
## Chapter 6: Session Modes (Claude Code) and Codex Policy

Session modes control how the active coding agent handles tool execution -- whether it asks for permission before making changes or acts autonomously. The control surface looks different per agent:

- **Claude Code** sessions use the **Mode Selector** (six modes from Normal to Bypass).
- **Codex** sessions use the **Codex Policy pill** (sandbox × approval-policy, plus a read-only network indicator).

CodeMantis swaps these controls in the input toolbar automatically based on the active session's `agent_id`.

---

### Claude Code: Mode Selector
...
### Codex: Policy Pill (sandbox × approval-policy)

Codex doesn't have a single "mode" axis. Its sandbox (what the agent is allowed to touch) and its approval policy (when it asks before acting) are independent, and CodeMantis surfaces both as one pill. The pill label uses the form `<sandbox> · <approval> ▾`, e.g. `workspace-write · on-request ▾`. Clicking the pill opens a popover with two radio groups.

**Sandbox options**

| Sandbox | What it allows |
|---|---|
| `read-only` | Agent can read but cannot write or run mutating commands. |
| `workspace-write` *(default)* | Writes inside the project tree; protected paths (`.git`, `.codex`, `.agents`) stay read-only. |
| `danger-full-access` | No sandbox at all. Use only for trusted scripts. |

**Approval-policy options**

| Approval | When Codex asks |
|---|---|
| `never` | Skip all prompts. Pair only with a strict sandbox. |
| `on-request` *(default)* | Codex asks before sandboxed-block commands or anything outside the trusted set. |
| `untrusted` | Mutations always require approval; reads run automatically. |

**Network access** appears below the radio groups as a *read-only* indicator (e.g. "Network: off"). Toggling it requires editing `~/.codex/config.toml` directly -- the `codex_network_access` Preflight recipe (Chapter 19B) walks you through that change. The indicator is read-only in v1.5.x because Codex evaluates network access from the config file at session start.

**Behaviour**

- Changes commit immediately via the `setCodexPolicy` IPC and update local state optimistically. On failure, the previous values are restored.
- Sandbox/approval changes take effect on the **next `turn/start`** -- they don't kill the in-flight turn.
- `Shift Tab` cycles through a small set of common presets so you can flip Codex's posture quickly without opening the popover. `Cmd .` does the same.
- Codex's four server-initiated approval kinds (`commandExecution`, `fileChange`, `mcpServer/elicitation`, `permissions`) all route onto the **same** Tool Approval modal as Claude (Chapter 7) -- only the JSON-RPC `result` shape on the way back differs.

**Tips**

1. **Start strict, relax as you trust.** `read-only · untrusted` is the safest pairing for an unfamiliar repo. Move to `workspace-write · on-request` once you've watched Codex behave on a few turns.
2. **`workspace-write · never` is the Codex equivalent of "Don't Ask" mode.** Fast for trusted refactors, but the protected-path guardrail (`.git`, `.codex`, `.agents`) is the only safety net.
3. **`danger-full-access` is genuinely dangerous** -- Codex can touch any file on disk and run arbitrary commands. Reserve it for sandboxes (containers, VMs, scratch directories).
...
**Codex approval kinds.** Codex's `app-server` raises four server-initiated approval requests, all of which route onto the existing Tool Approval modal:

- `commandExecution` -- Codex is about to run a shell command that requires approval under the active policy.
- `fileChange` -- Codex wants to write to a file. Edits to protected paths (`.git`, `.codex`, `.agents`) come through here.
- `mcpServer/elicitation` -- An MCP server attached to Codex is asking for a piece of input via elicitation.
- `permissions` -- Codex is asking to escalate its sandbox or approval policy for the current turn.

You won't see any difference in the modal UI -- the same Approve / Deny / Always-allow buttons handle all four. The denial reason ("Denied by user") is delivered back to Codex in the right JSON-RPC `result` shape automatically.

**Multi-agent protected-path detector.** When the active agent denies a write because it landed in a protected directory, the chat shows a bucketed toast naming the responsible guardrail:

- **Claude Code:** `.claude/` `.git/` `.vscode/` → "Claude CLI's protected-path guardrail."
- **Codex:** `.codex/` `.git/` `.agents/` → "Codex's sandbox."

The toast collapses repeated denials into a single line per bucket so a noisy session doesn't paper the chat.
...
### Agent identity is preserved across history and recovery (v1.5.x)

Every persisted session record carries its `agent_id` (`claude_code` or `codex`) alongside the conversation. That metadata is the single source of truth used by:

- **Session History UI** -- the right-side history list shows the correct agent badge (CC / CX) per row, even after an app restart, and resuming a Codex session re-spawns it on Codex (never on Claude, regardless of what the *current* primary or per-task default is in Settings → Agents).
- **Wake recovery** -- after macOS App Nap or sleep, when CodeMantis reloads to reattach live sessions, the agent identity rides through the reload. A Codex session you left running before sleep is still a Codex session when it comes back.
- **Crash recovery** -- if the app force-quits, the next launch's recovery pass replays the right adapter per session. There is no "default to Claude" fallback that would silently re-route a Codex session.

The practical consequence: per-task routing in Settings → Agents only affects *new* sessions, never existing ones. To move an open session to a different agent, close it and start a new one.
...
## Chapter 19B: Preflight & Mission Control

Preflight is CodeMantis's per-project capability gate. Before you start coding -- and before Self-Drive begins an autonomous run -- Preflight verifies that every service, secret, and CLI tool the project actually needs is in place. The user-facing surfaces are the always-visible **PreflightTray** strip and the full-page **Mission Control** setup wizard.
...
Slash commands let you trigger actions by typing "/" in the chat input. The Command Palette shows all available commands with categories, descriptions, and keyboard navigation. For commands that require the interactive CLI (like `/model` / `/config` on Claude, or `/login` / `/mcp` / `/apply` on Codex), the CLI Overlay provides a full terminal session running the active session's agent binary.
...
If a command was routed from the palette, it is auto-typed (Claude) or spawned as argv (Codex) after the prompt is ready.
...
Skill / Prompt commands are discovered from agent-specific directories at both project and user levels:

- **Claude Code sessions:** `.claude/commands/` (flat `.md` prompts) and `.claude/skills/` (per-skill directories containing `SKILL.md`); project entries override user entries with the same name.
- **Codex sessions:** `.codex/prompts/` (flat `.md` prompts); project entries override user entries with the same name. Codex does not have a "skills" concept.

There are no dedicated settings for the command palette or CLI Overlay.
...
A panel with the heading **"AI Providers"** and the subtext: "Configure API keys and token pricing for each provider. These are shared across Changelog, Assistant, Super-Bro, and SpecWriter features."

> 🔒 **API keys are encrypted at rest** (v1.1.10+). The values you enter are sealed with **AES-GCM** via the new `secret_box` Rust module and per-user key material, then stored in `settings.json` as `api_keys_encrypted`. The legacy plaintext `api_keys` field is migrated opportunistically on next save. Keys remain decrypted only in process memory while the app runs.
...
**Model Pricing** -- Below a divider labeled "Model Pricing (per 1M tokens, USD)", each provider's models are listed with editable **In** (input) and **Out** (output) cost fields (v1.5.x lineup):
...
## Chapter 27A: Settings -- Agents (per-task routing)

The Agents tab (new in v1.3.0, expanded in v1.5.0) is where you see which coding-agent CLIs CodeMantis has detected and choose which one new sessions use by default. From v1.5.0 onward you can also route specific *task categories* (main chat, Assistant panel, SpecWriter, Help) to different agents -- so you can write specs with Claude Code while routing your main chat through Codex, for example, and split your subscription pool across the two agents.

### Why per-task routing exists (the 15 June 2026 billing change)

From **15 June 2026** Anthropic moves headless usage of the Claude CLI -- anything driven through `claude -p` or the Agent SDK, which is how Self-Drive, SpecWriter, and CodeMantis's Help assistant talk to Claude under the hood -- onto a **metered credit pool** that is *separate from your interactive Claude Pro / Max subscription*. Interactive chat (you typing into a session) still draws from the subscription as before; programmatic / headless chat now bills against credits. The OpenAI Codex CLI is unaffected: it keeps drawing from your **ChatGPT Plus / Pro / Business subscription** regardless of how it's invoked.

The practical consequence is that the same prompt costs different things on different agents depending on whether you're typing it or whether CodeMantis is dispatching it on your behalf. Per-task routing lets you decide which pool each kind of work lands in:
...
- Route **SpecWriter**, **Help**, or the **Assistant panel** to Codex to push their (usually heavier, often headless) traffic onto your ChatGPT subscription instead of into Anthropic's credit pool.
- Or invert the split: keep iterative spec work on Claude and push your main chat through Codex if your ChatGPT plan has more headroom.

The **Usage split (last 7 days)** panel at the bottom of the tab tells you, in honest session counts (not dollars -- CLI sessions are subscription-billed, so an invented $ figure for Codex would be misleading), where your traffic is actually landing.

> **Self-Drive note.** Self-Drive is intentionally *not* a per-task category. It attaches to a user-created session and inherits whichever agent that session is on -- it never spawns its own session, so a "Self-Drive → Codex" dropdown would route nothing. If you want Self-Drive to run on Codex, route **Main chat** to Codex (or open the session manually with Codex selected) and start Self-Drive from there.
...
- The agent name and a tagline ("Anthropic's CLI -- uses your Claude Pro/Max subscription." / "ChatGPT-bundled coding agent -- uses your ChatGPT subscription, no separate OpenAI API key needed.").
...
A small table mapping task categories to agents. Categories:

| Task | What it controls |
|---|---|
| `main_chat` | Which agent powers new sessions you create from the title bar |
| `assistant` | Which local-CLI assistant (Claude Code / Codex) tabs default to in the Assistant panel |
| `spec_writer` | Which agent SpecWriter spawns for the "Claude Code" / "Codex" provider option |
| `help` | Which agent the in-app Help assistant uses |

Each row's value is one of:

- **"Use primary"** (default) -- inherits whichever agent is currently `selectedAgentId`. The sparse map intentionally omits entries that fall back to the primary.
- **"Claude Code"** -- pins this task to Claude regardless of the primary.
- **"OpenAI Codex"** -- pins this task to Codex regardless of the primary.

A **Reset to defaults** button clears all per-task overrides at once.

> **Self-Drive is *not* a task category.** Self-Drive always inherits the agent of the session it attaches to -- if your `main_chat` is routed to Codex, Self-Drive will run on Codex too. The pre-run gate (Preflight) and orchestrator preamble still apply, but the agent is fixed by the underlying session.

**Usage split (last 7 days)** (v1.5.0)
...
- Estimated cost (computed against the same Model Pricing table the Cost Log uses).
- A small horizontal bar chart that visualises the Claude vs. Codex split.

The breakdown is read-only -- it exists so you can see, at a glance, whether your routing decisions match where you actually want the credit spend to land.

### How to Open / Access

| Method | Action |
|---|---|
| Settings modal | Click the **"Agents"** tab (Bot icon) in the left sidebar |
| Title bar | Open Settings (`Cmd ,`), then "Agents" |

### User Actions

**Promote an agent to default**

Click **"Make default"** on a row. Subsequent new sessions will spawn with that agent unless a per-task override pins them somewhere else.

**Route a task to a specific agent**

In the Per-task Defaults table, change the dropdown for `main_chat`, `assistant`, `spec_writer`, or `help` to a specific agent. The change is persisted to `defaultAgentByTask` in settings as a sparse map (the dropdown stays on "Use primary" if you don't pick a specific agent).

**Reset task routing**

Click **Reset to defaults** -- the sparse map is cleared and every category falls back to the primary agent.

**Refresh detection**

Detection runs once on tab open. To refresh after installing a missing CLI, leave Settings and reopen the Agents tab; a fresh `checkClaudeStatus` / `checkCodexStatus` IPC call runs on mount.

### States

- **Both installed:** Both rows show green install + auth badges. Per-task Defaults table is visible.
- **One installed:** The other row shows "Not installed" with its install command. Per-task Defaults table is hidden (it only makes sense when there is more than one agent to choose).
...
- **Settings path:** Settings > Agents > Default agent (`selectedAgentId`).
- **Settings path:** Settings > Agents > Per-task Defaults (`defaultAgentByTask`, sparse).
- **Internal:** Detection IPC calls are `checkClaudeStatus()` and `checkCodexStatus()`. Both return `{ installed, version, parsed_version, authenticated }`.

### Tips

1. The cheapest way to stretch a Claude Pro subscription is to route only **SpecWriter** and **Help** to Codex (they're heavy on prose, light on tool use) while keeping Claude as your primary. The 7-day Usage Split tells you whether that's actually saving you anything.
2. If a Codex session refuses to start with "Authentication required", check the Agents tab -- the auth badge tells you whether the problem is install vs. login vs. detection. Run `codex login` in a terminal; the badge updates next time you open the tab.
3. Promoting an agent to default via "Make default" only affects *new* sessions. Already-open sessions stay bound to whichever agent they were created with -- agents are per-session, not per-app.
...
### Crash Recovery (force-quit, hard restart)

**What happens:** If the app force-quits, panics, or the OS kills the process while one or more sessions are running, CodeMantis can resume those sessions on next launch instead of losing the conversation.

**How it works (v1.1.8 / v1.1.10):**

- The CLI session id (Claude's `--resume` token, or Codex's `thread_id`) is persisted to SQLite on the very first `system`/`init` event of each session. So even a session that crashed *seconds* after starting is recoverable.
- `agent_id` is persisted alongside the session id; Resume routes Claude sessions through `--resume` and Codex sessions through `thread/resume` automatically.
- A `mark_session_closed_if_stale` IPC + snapshot-tick reconciliation cleans up sessions whose host process is gone, so the Resume list never offers ghost rows.
- On launch, a **crash-recovery toast** appears (e.g. *"Recovered 2 sessions, 1 could not be restored"*) and the Session History tab gets a **Restore** action for paused-recovered rows.
- If the agent was streaming when the crash happened, the session reopens in a **paused-recovered** state with explicit *Restore* / *Resume* actions -- nothing replays automatically until you decide.

### Wake Recovery (post-suspend, post-screen-lock)

**What happens:** On macOS, the WKWebView's render pipeline can stall when the system suspends or the screen locks for long enough. v1.1.4 opts CodeMantis out of App Nap and holds a process-activity-assertion token; v1.1.10 adds a **graduated wake recovery** strategy on top.

**Behaviour:**

- A heartbeat sends `wake_ping` from the main process and waits for `wake_pong` from the renderer.
- **Tick 1 missed:** the title bar repaints (cheap nudge).
- **Ticks 2--4 missed:** repaint + webview eval to wake up `WebContent`.
- **Tick 5+ missed:** the renderer is reloaded as a last resort, and the streak resets after the reload.
- Breadcrumbs (including the missed-pong streak count) are written to `wake.log` for post-mortem analysis.
- The reload preserves project + session selection (the recent-state tail in the database is replayed). Landing on the start screen after a wake is a **bug** -- file an issue.
...
| Command | Category | Description | Argument Hint |
|---|---|---|---|
| *(varies)* | Skill | **Claude Code sessions:** Custom commands from `.claude/commands/` (flat `.md` files) and `.claude/skills/` (per-skill directories with `SKILL.md`). **Codex sessions:** Prompts from `.codex/prompts/` (flat `.md` files). All are discovered from both project-level and user-level (`~/.claude/` or `~/.codex/`) directories; project entries override user entries with the same name. | Defined per skill via frontmatter |
...
|---|---|---|---|
| Claude Code (local) | CC (purple) | No -- uses Claude Pro/Max subscription | Assistant (with tool access), SpecWriter, Main chat |
| Codex (local) | CX (teal) | No -- uses ChatGPT Plus/Pro/Business subscription | Assistant (with tool access), SpecWriter, Main chat |
...
#### Codex CLI Models (Main Chat)

When a session is on Codex, the model selector reflects whatever Codex's `initialize` response advertises (typically `gpt-5.5` and `gpt-5.5-mini`). Codex doesn't expose per-session model selection in v1.5.x — the dropdown shows the account default as a read-only label. Codex models that declare `supportsEffort` (e.g. `gpt-5.5`) render the **Effort selector** with their advertised levels and `defaultEffort`; on Codex, effort changes apply **per turn** instead of requiring a process restart.

#### SpecWriter Models (Priority Order)

Auto-select walks this list in order and picks the first model whose provider has a configured API key. The default is **Gemini 3.5 Flash** (`gemini-3.5-flash`).
