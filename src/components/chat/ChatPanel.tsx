...
export default function ChatPanel() {
  ...
  const { startSession, resumeRecoveredSession, reviveCodexSession, freshThreadCodexSession } = useClaudeSession();
  ...
  // Primary recovery: revive the same Codex thread non-destructively (resume,
  // full context preserved). For a wedged connection / lost notification.
  const handleRecover = useCallback(() => {
    if (!session) return;
    reviveCodexSession(session.id).catch((e) =>
      console.error("Failed to recover session:", e)
    );
  }, [session, reviveCodexSession]);

  // Escalation: a revive already failed → start a fresh empty thread + recap.
  const handleFreshThread = useCallback(() => {
    if (!session) return;
    freshThreadCodexSession(session.id).catch((e) =>
