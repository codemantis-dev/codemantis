...
>
> - **OpenAI Codex is now a first-class agent** alongside Claude Code (v1.3.0). The Project Picker shows an Agent Picker when both `claude` and `codex` are installed; sessions carry an `agent_id` and the input toolbar shows a **Codex Policy pill** instead of the Mode selector for Codex sessions.
...
> - **Self-Drive evidence vocabulary + loop guard + capability gating** (v1.1.11): the orchestrator parses Claude's verify replies against a typed evidence vocab, bounded retry on repeat-pattern failures, and verify items tagged `capability=<id>` auto-resolve as N/A when the capability is absent.
> - **AES-GCM encryption of AI provider API keys at rest** (v1.1.10).
> - **Persistent CLI session id, crash recovery + graduated wake recovery** (v1.1.8 / v1.1.10): force-quit sessions are resumable, post-suspend wake re-attaches live sessions and reloads if necessary.
> - **Refreshed Assistant model lineup** (v1.3.x): Gemini 3.x preview, GPT-5.5 family, Opus 4.8 / Sonnet 4.6 / Haiku 4.5; retired model IDs dropped.
...
The Welcome Screen fills the entire window. At the top center is the CodeMantis icon (a large rounded square logo) followed by the heading **"Welcome to CodeMantis"**, the subtitle *"the native macOS app Claude Code and Codex deserve"*, and the current version number (e.g., v1.3.0).
...
**Network access** appears below the radio groups as a *read-only* indicator (e.g. "Network: off"). Toggling it requires editing `~/.codex/config.toml` directly -- the `codex_network_access` Preflight recipe (Chapter 19B) walks you through that change. The indicator is read-only in v1.3.x because Codex evaluates network access from the config file at session start.
...
- **OpenAI:** Requires an API key. Available models in v1.3.x: **GPT-5.4 Mini**, **GPT-5.4**, **GPT-5.5**. Retired model IDs (GPT-4.1, GPT-5.4 Nano) were dropped after Anthropic / OpenAI removed them from their public APIs.
- **Google Gemini:** Requires an API key. Available models in v1.3.x: **Gemini 2.5 Flash Lite**, **Gemini 2.5 Flash**, **Gemini 3.1 Flash Lite**, **Gemini 3.5 Flash**, **Gemini 3.1 Pro**. (Gemini 2.5 Pro and the 3.0 Flash preview were dropped after Google retired them.)
...
- **Model selector** -- lists the available models for the selected provider. For Claude Code, the choices are Haiku 4.5 (fast, lower cost), Sonnet 4.6 (balanced — default), and Opus 4.8 (highest quality). For Codex, SpecWriter uses the account default and shows it as a read-only label (Codex doesn't expose per-session model selection in v1.3.x).
...
**Agent support.** Self-Drive runs on either Claude Code or Codex sessions. In v1.3.0 the **build-mode preamble** is Claude-tuned (evidence-vocab vocabulary, recheck rules, parity-gate phrasing) — Codex runs work but get a slightly less precise verify pass; a Codex-tuned preamble variant is planned for v1.4.0.
...
**Model Pricing** -- Below a divider labeled "Model Pricing (per 1M tokens, USD)", each provider's models are listed with editable **In** (input) and **Out** (output) cost fields (v1.3.x lineup):
...
| Codex policy per session | (Input toolbar) | `CodexSessionPolicy` | `workspace-write · on-request` | Per-Codex-session sandbox × approval policy, stored in `uiStore.codexPolicies[sessionId]`. Network access is read-only in v1.3.x and requires editing `~/.codex/config.toml`. |
...
When a session is on Codex, the model selector reflects whatever Codex's `initialize` response advertises (typically `gpt-5.5` and `gpt-5.5-mini`). Codex doesn't expose per-session model selection in v1.3.x — the dropdown shows the account default as a read-only label. Codex models that declare `supportsEffort` (e.g. `gpt-5.5`) render the **Effort selector** with their advertised levels and `defaultEffort`; on Codex, effort changes apply **per turn** instead of requiring a process restart.
