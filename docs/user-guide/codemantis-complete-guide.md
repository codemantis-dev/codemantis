...
<!-- Date: April 2026 | App Version: 1.0.2 -->

# CodeMantis -- Complete User Guide

> This guide covers every feature, every button, every setting, and every keyboard shortcut in CodeMantis v1.0.2. It was generated by reading the application's source code -- every description matches the actual implementation.
...
The Welcome Screen fills the entire window. At the top center is the CodeMantis icon (a large rounded square logo) followed by the heading **"Welcome to CodeMantis"**, the subtitle *"Native desktop UI for Claude Code"*, and the current version number (e.g., v1.0.2).
...
See [Chapter 21B: Self-Drive](#chapter-21b-self-drive----autonomous-implementation) for full details on configuration and usage.
